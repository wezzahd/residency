...
void main()  {
    ...
    vec2 mousepos = gl_FragCoord.xy - (resolution.xy) * vec2(1.-0.16,0.5);

	float d2 = ComputeCircle(mousepos, center, 100.0 , 200.);
...
vec3 texColor = hsb2rgb(vec3(1.0,1.0,1.0));
...
float edge2 = ((radius*2.0) * (1.5));
  ...
  vec2 pos = mod(screenPos * d2, vec2(diameter)) - vec2(radius);
