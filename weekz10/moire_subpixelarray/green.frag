...
void main()  {
    ...
    vec2 mousepos = gl_FragCoord.xy - (resolution.xy) * vec2(0.5,0.5);

	float d2 = ComputeCircle(mousepos, center, 200.0 , 10.);
...
float edge2 = ((radius*2.0) * (1.5));
  ...
  vec3 greenmaskout = mix(greenmask,bg,1.-noisecolor.x);
