		...
		//vec2 st = gl_FragCoord.xy/resolution.xy;
			noiseuv.x *= resolution.x/resolution.y;
				...
				a = snoise(posi*vec2(cos(u_time*0.15),sin(u_time*0.1))*0.1)*3.1415;
						...
						noisecolor = vec3( smoothstep(0.,1.0,fract(DF)) );
...
vec3 texColor = hsb2rgb(vec3(1.0-noisecolor.x,1.-noisecolor.y,1.0));

	//vec3 alphmask = texture2D(tex1, uv, -32.0).rgb; //pixellate alphamask

	float edge = ((radius*2.0/3.0) + (radius * texColor.x)); // random sizes // generate different si
