...
//screenPos3.x *= aspect;
 ...
 //	float pct = 1.0- length(center - screenPos2);




		float edge = radius/30.;//* (1.-d2) ;//random(uv) * radius/2.0; // random sizes // generate different si

//float test = (radius/2.0) * step(pct,0.2) + (edge) * step(pct,0.2);
...
//uv.y = (mobiletest) - uv.y; // flip UV if on Desktop
	...
	vec3 redmask = mix(col,bg,1.-mask.x);
