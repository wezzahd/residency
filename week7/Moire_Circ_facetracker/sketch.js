...
// the shader variable
let redShader,greenShader,blueShader,bgshader;
...
function preload(){
  ...
  bgShader = loadShader('effect.vert', 'bg.frag');
}
...
//  imageaspectratio(campg);
  ...
  bgShader.setUniform('tex0', cam);
  bgShader.setUniform('resolution', [width, height]);
  bgShader.setUniform('tileno', tileno);
  bgShader.setUniform('radius', radius);
...
bgpg.resizeCanvas(windowWidth, windowHeight);
