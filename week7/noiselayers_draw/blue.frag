...
//uv.y = (mobiletest) - uv.y; // flip UV if on Desktop
	...
	float edge = radius/4.0 + (radius*1.5 * noisecolor2.x  * sin( uv.y)  * texColor.x) ;//(radius * (noisecolor.x) * noisecolor2.x); // random sizes // generate different si
// float edge2 = radius;
//
...
//	float d = box(screenPos2, vec2(edge) , 0.5);
    ...
    vec3 col = mix(vec3(1.0), bg, (d)); //1.-d for rect
		...
		float lerpcol = lerp(col.z, col.z * noisecolor2.x, texColor.x);
    vec3 red = vec3(0.0,0.0,lerpcol);
