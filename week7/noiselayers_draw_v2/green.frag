...
vec3 noiser (vec2 pos, float factor) {
	...
	a = snoise(posi*vec2(cos(u_time*0.15),sin(u_time*0.1))*0.1)*3.1415;
			...
			return vec3( smoothstep(0.5,1.,fract(DF)) );
}
...
vec3 noisecolor = noiser(uv,1.0);
vec3 noisecolor2 = noiser(st,10.0);
...
//uv.y = (mobiletest) - uv.y; // flip UV if on Desktop
	...
	float edge = radius/4.0 + (radius*2.0 * noisecolor2.x * texColor.x);//(radius * (noisecolor.x) * noisecolor2.x); // random sizes // generate different si
// float edge2 = radius;
//
...
//	float d = box(screenPos2, vec2(edge) , 0.5);
    ...
    vec3 col = mix(vec3(1.0), bg, (d)); //1.-d for rect
		...
		vec3 red = hsb2rgb(vec3(1.-noisecolor.y,1.0-noisecolor.y,1.0));

		vec3 red2 = mix(red, bg, noisecolor.x);

		float lerpcol = lerp(col.y, col.y * red2.y , texColor.x);
    ...
    // Set the final fragment color.
	  gl_FragColor = vec4(vec3(0.0,lerpcol,0.), 1.0);
