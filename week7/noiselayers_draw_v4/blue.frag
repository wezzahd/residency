...
//	float d = box(screenPos2, vec2(edge) , 0.5);
    ...
    vec3 col = mix(vec3(1.0), bg, (d)); //1.-d for rect
		...
		vec3 red = hsb2rgb(vec3(1.-noisecolor.z,1.-noisecolor.y,1.0));

		vec3 red2 = mix(red, bg, noisecolor.x);

		float lerpcol = lerp(col.z, col.z * red2.z , texColor.x);
    ...
    // Set the final fragment color.
	  gl_FragColor = vec4(vec3(0.0,0.0,lerpcol), 1.0);
