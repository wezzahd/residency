...
// Compute the relative distance to the circle, where < 0.0 is outside the feathegreen border,
// and > 1.0 is inside the feathegreen border.
...
void main()  {
    ...
    // A feather value (in pixels) is used to greenuce aliasing artifacts when the circles are small.
...
//uv.y = (mobiletest) - uv.y; // flip UV if on Desktop
    ...
    vec3 green = vec3(0.,col.y,0.);
		...
		//float greenmask = col.x * step(mask.x,0.99) + 0.0 * step(0.99,mask.x);
