...
// Compute the relative distance to the circle, where < 0.0 is outside the featheblue border,
// and > 1.0 is inside the featheblue border.
...
void main()  {
    ...
    // A feather value (in pixels) is used to blueuce aliasing artifacts when the circles are small.
...
//uv.y = (mobiletest) - uv.y; // flip UV if on Desktop
    ...
    vec3 blue = vec3(0.,0.,col.z);
		...
		//float bluemask = col.x * step(mask.x,0.99) + 0.0 * step(0.99,mask.x);
